...
- 服务端：`tximsign.php` 或登录响应提供 `userSign`；产品 Web 仅向获得“非匹配主动私信”全局授权的账号下发服务端 UserSig。普通账号不会获得浏览器直连凭证。
- 受控收发：普通账号的好友、匹配和已有会话通过 BFF 逐对象鉴权后发送 REST 文本或 5 秒闪图；鉴权前同步本人黑名单和 `getBlackListMe` 返回的反向黑名单，任一方向都优先覆盖好友、匹配、已有会话和全局主动私信授权。首次同步失败时关闭私聊并暂停下发 UserSig，已有快照刷新失败时继续保留最近一次有效阻断。
- 实时收发：获得全局主动私信授权的浏览器可使用固定版本 TIM SDK 和上传插件发送文字及富媒体。UserSig 是账号级凭证，无法限定单个聊天对象，因此该直连通道不经过 BFF 的逐对象鉴权；撤销全局授权后前端会停止直连并切回受控通道。
