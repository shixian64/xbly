...
| **消息** | 聚焦历史会话列表、未读数与会话双栏；通讯录可从关系中心或消息空态进入；私信发送统一经过 BFF 实时鉴权，支持受控文本发送、历史同步和 5 秒闪图 |
...
| IM 实时体验 | 产品 Web 不下发腾讯 UserSig 或融云 Token，避免管理员撤销权限后旧凭证绕过 BFF；文本发送统一经 BFF 动态检查主动私信、匹配关系或已有归档会话权限，并通过腾讯 REST 投递；接收与未读状态通过历史会话同步刷新 |
