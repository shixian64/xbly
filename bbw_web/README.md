...
| **消息** | 聚焦历史会话列表、未读数与会话双栏；通讯录可从关系中心或消息空态进入；浏览器通过腾讯 TIM SDK 支持文字、图片、动图、语音、视频、文件、表情包和 5 秒闪图 |
...
| IM 实时体验 | 产品 Web 向已登录浏览器下发服务端 UserSig，并使用固定版本 TIM SDK 和上传插件实时收发文字及富媒体；SDK 连接失败时回退到经过 BFF 权限检查的 REST 文本通道。浏览器直连 TIM 不经过 BFF 的逐对象私信鉴权，管理员停用本站会话也不能立即吊销已经建立的上游 TIM 会话 |
